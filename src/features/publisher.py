...
try:
    ...
    from ..data_models import AccountConfig, LLMSettings, ScrapedTweet, TweetContent
    ...
    from src.data_models import AccountConfig, LLMSettings, ScrapedTweet, TweetContent
...
class TweetPublisher:
    def __init__(
        ...
        self.twitter_automation_settings = self.config_loader.get_settings().get(
            "twitter_automation", {}
        )
        self.media_dir = self.twitter_automation_settings.get(
    ...
    async def _download_media(self, media_url: str) -> Optional[str]:
        ...
        try:
            ...
            if not base_name or "." not in base_name:  # Fallback if no filename in URL
                ...
                ext = (
                    mimetypes.guess_extension(content_type) if content_type else ".jpg"
                )
            ...
            logger.error(
                f"An unexpected error occurred during media download from {media_url}: {e}"
            )
            return None

    async def post_new_tweet(
        self, content: TweetContent, llm_settings: Optional[LLMSettings] = None
    ) -> bool:
        """
        ...
        logger.debug(f" Posts a new tweet llm_settings  {llm_settings}")
        ...
        if llm_settings and (
            "generate tweet about" in tweet_text.lower()
            or "write a post on" in tweet_text.lower()
        ):  # Simple check
        ...
        final_media_paths = [
            os.path.abspath(p) for p in final_media_paths if os.path.exists(p)
        ]

        logger.info(
            f"Attempting to post tweet: '{tweet_text[:50]}...' with {len(final_media_paths)} media file(s)."
        )
        ...
        try:
            ...
            if final_media_paths:
                ...
                try:
                    add_media_button = self.driver.find_element(
                        By.XPATH, '//button[@data-testid="mediaButton"]'
                    )
                    self._force_click(add_media_button)
                    time.sleep(1)  # Wait for file dialog to be ready (conceptually)
                except NoSuchElementException:
                    logger.debug(
                        "Did not find a separate 'Add Media' button, proceeding to file input."
                    )
    ...
    async def reply_to_tweet(
        self, original_tweet: ScrapedTweet, reply_text: str
    ) -> bool:
        """
        ...
        if not original_tweet.tweet_url:
            logger.error(
                f"Cannot reply to tweet {original_tweet.tweet_id}: Missing tweet URL."
            )
            return False
        if not reply_text:
            logger.error(
                f"Cannot reply to tweet {original_tweet.tweet_id}: Reply text is empty."
            )
            return False
        ...
        try:
            ...
            reply_icon_button = WebDriverWait(main_tweet_element, 10).until(
                ...
                )  # Relative to the tweet article
            )
            self._force_click(reply_icon_button)
            ...
            try:
                ...
                TimeoutException
            ):  # Fallback if not in a typical modal layer structure
                logger.debug(
                    "Reply composer not found in standard modal layer, trying general tweetButton."
                )
                reply_post_button = WebDriverWait(self.driver, 10).until(
                    EC.element_to_be_clickable((By.XPATH, reply_post_button_xpath))
                )

            self._force_click(reply_post_button)
            ...
            logger.info(
                f"Reply to tweet {original_tweet.tweet_id} posted successfully."
            )
            return True
    ...
    async def retweet_tweet(
        ...
        if not original_tweet.tweet_url:  # Need URL to navigate to the tweet
            logger.error(
                f"Cannot retweet tweet {original_tweet.tweet_id}: Missing tweet URL."
            )
            return False
        ...
        if (
            is_quote_tweet
            and llm_settings_for_quote
            and (
                "generate quote for" in quote_text_prompt_or_direct.lower()
                or "write a quote about" in quote_text_prompt_or_direct.lower()
            )
        ):
            logger.info(
                f"Generating quote text for tweet {original_tweet.tweet_id} using prompt: {quote_text_prompt_or_direct}"
            )
            ...
            if not generated_quote:
                logger.error(
                    f"Failed to generate quote text for tweet {original_tweet.tweet_id}. Aborting retweet."
                )
                return False
        ...
        logger.info(
            f"Attempting {action_type_log} for tweet ID: {original_tweet.tweet_id}"
        )
        if final_quote_text:
        ...
        try:
            ...
            retweet_button_xpath = (
                './/button[@data-testid="retweet"]'  # Relative to tweet article
            )
            ...
            self._force_click(retweet_icon_button)
            ...
            if is_quote_tweet:
                ...
                try:
                    ...
                    quote_option = WebDriverWait(self.driver, 5).until(
                        EC.element_to_be_clickable(
                            (
                                By.XPATH,
                                '//div[@data-testid="Dropdown"]//a[contains(@href,"/compose/tweet")]',
                            )
                        )
                    )  # Common pattern for quote tweet link

                self._force_click(quote_option)
                logger.info("Clicked 'Quote' option.")
                time.sleep(2)  # Wait for quote tweet composer to appear

                # Composer text area for quote
                quote_text_area_xpath = (
                ...
                try:
                    confirm_retweet_button = WebDriverWait(self.driver, 5).until(
                        EC.element_to_be_clickable(
                            (By.XPATH, '//button[@data-testid="retweetConfirm"]')
                        )  # Common testid
                    )
                except TimeoutException:
                    confirm_retweet_button = WebDriverWait(self.driver, 5).until(
                        EC.element_to_be_clickable(
                            (
                                By.XPATH,
                                '//div[@role="menuitem" and contains(., "Repost")]//div[1]',
                            )
                        )  # Click the div if span is tricky
                    )
                self._force_click(confirm_retweet_button)
            ...
            logger.info(
                f"{action_type_log} for tweet {original_tweet.tweet_id} successful."
            )
            return True
...
if __name__ == "__main__":
    ...
    async def test_publisher():
        ...
        if not accounts:
            logger.error(
                "No accounts configured in config/accounts.json. Cannot run publisher test."
            )
            return
        ...
        try:
            ...
            mock_account_model = AccountConfig(
                account_id=active_account_config_dict.get(
                    "account_id", "test_publisher_user"
                ),
                cookie_file_path=active_account_config_dict.get(
                    "cookie_file_path"
                ),  # BrowserManager uses this
            )
        ...
        bm = BrowserManager(
            account_config=active_account_config_dict
        )  # BrowserManager takes the dict
        ...
        publisher = TweetPublisher(
            browser_manager=bm, llm_service=llm, account_config=mock_account_model
        )

        try:
            logger.info(
                f"Testing publisher for account: {mock_account_model.account_id}"
            )

            # Test 1: Post a simple text tweet
            simple_content = TweetContent(
                text="Hello from the automated world! This is a test tweet. #Python #Automation"
            )
            ...
            gen_llm_settings = LLMSettings(
                service_preference="gemini", max_tokens=100, temperature=0.8
            )  # Prefer Gemini

            prompt_content = TweetContent(text=llm_prompt)  # Text here is the prompt
            logger.info("\n--- Testing LLM-generated post ---")
            success_llm = await publisher.post_new_tweet(
                prompt_content, llm_settings=gen_llm_settings
            )
