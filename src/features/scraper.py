...
class TweetScraper:
    def __init__(
        self, browser_manager: BrowserManager, account_id: Optional[str] = None
    ):
        ...
        self.scrape_settings = self.config_loader.get_twitter_automation_setting(
            "scraper_config", {}
        )
        self.default_max_tweets = self.config_loader.get_twitter_automation_setting(
            "max_tweets_per_scrape", 50
        )
        self.scroll_delay_min = self.scrape_settings.get(
            "scroll_delay_min_seconds", 1.5
        )
        self.scroll_delay_max = self.scrape_settings.get(
            "scroll_delay_max_seconds", 3.5
        )
        self.no_new_tweets_scroll_limit = self.scrape_settings.get(
            "no_new_tweets_scroll_limit", 5
        )  # Stop after 5 scrolls with no new tweets
    ...
    def _parse_tweet_card(self, card_element: WebElement) -> Optional[ScrapedTweet]:
        ...
        try:
            ...
            # Extract user handle
            user_handle_element = card_element.find_element(
                By.XPATH,
            ...
            # Extract tweet text
            tweet_text_parts = []
            text_elements = card_element.find_elements(
                By.XPATH,
            ...
            for el in text_elements:
                try:
                    ...
                    StaleElementReferenceException
                ):  # Element might go stale during iteration
                    logger.warning(
                        "Stale element reference when extracting tweet text part."
                    )
                    continue
            text_content = "".join(tweet_text_parts).strip()
            if (
                not text_content
            ):  # Skip if no text content, might be a poll or other non-text tweet
                return None
            ...
            try:  # Example selector, adjust based on actual X.com structure
                view_element = card_element.find_element(
                    By.XPATH,
            ...
            tags = [
                tag.text
                for tag in card_element.find_elements(
                    By.XPATH, './/a[contains(@href, "src=hashtag_click")]'
                )
            ]
            ...
            mentions = [
                mention.text
                for mention in card_element.find_elements(
                    By.XPATH,
            ...
            try:
                img_element = card_element.find_element(
                    By.XPATH, './/div[@data-testid="Tweet-User-Avatar"]//img'
                )
            ...
            if (
                user_handle
                and f"@{user_handle.lstrip('@')}" in text_content
                and reply_count == 0
            ):  # Replying to self often indicates a thread start
    ...
    def scrape_tweets_from_url(
        ...
        stop_if_no_new_tweets_count: Optional[
            int
        ] = None,  # How many scrolls with no new tweets before stopping
    ) -> List[ScrapedTweet]:
        ...
        # progress._display(0, False, 0, no_tweets_limit=(max_tweets == float("inf")))
            ...
            try:
                ...
                if not tweet_card_elements:
                    ...
                    if (
                        ...
                        logger.info(
                            f"No new tweets found after {stop_if_no_new_tweets_count} scrolls. Stopping."
                        )
                        break
                    # Try scrolling once more if nothing found initially
                    if not self.scroller.scroll_page():
                        break  # End of page or error
                    time.sleep(
                        random.uniform(self.scroll_delay_min, self.scroll_delay_max)
                    )
                    continue
                ...
                if new_tweets_found_this_scroll == 0:
                    ...
                    logger.info(
                        f"No new tweets found in this scroll. Attempt {scroll_attempts_with_no_new_tweets}/{stop_if_no_new_tweets_count}"
                    )
                else:
                ...
                if scroll_attempts_with_no_new_tweets >= stop_if_no_new_tweets_count:
                    logger.info(
                        f"Stopping scrape for {url}: No new tweets after {stop_if_no_new_tweets_count} consecutive empty scrolls."
                    )
                    break
                ...
                if (
                    not self.scroller.scroll_page()
                ):  # scroll_page returns False if end of page or error
                ...
                logger.error(
                    f"Unhandled exception during scraping {url}: {e}", exc_info=True
                )
                break
    ...
    def scrape_tweets_by_keyword(
        self, keyword: str, max_tweets: Optional[int] = None
    ) -> List[ScrapedTweet]:
    ...
    def scrape_tweets_from_profile(
        self, profile_url: str, max_tweets: Optional[int] = None
    ) -> List[ScrapedTweet]:
        # Profile URL is typically https://x.com/USERNAME
        return self.scrape_tweets_from_url(profile_url, "profile", max_tweets)

    def scrape_tweets_by_hashtag(
        self, hashtag: str, max_tweets: Optional[int] = None
    ) -> List[ScrapedTweet]:
...
if __name__ == "__main__":
    ...
    if not os.path.exists(dummy_cookie_file):
        with open(dummy_cookie_file, "w") as f:
            ...
            json.dump(
                [{"name": "test_cookie", "value": "test_value", "domain": ".x.com"}], f
            )
    ...
    if accounts:  # Try to use the first account if available
        ...
        logger.info(
            "No accounts configured in accounts.json, running scraper without specific account cookies."
        )

    bm = BrowserManager(account_config=test_account_cfg)
    scraper = TweetScraper(
        browser_manager=bm,
        account_id=test_account_cfg.get("account_id")
        if test_account_cfg
    ...
    try:
        ...
        keyword_tweets = scraper.scrape_tweets_by_keyword(
            keyword_to_scrape, max_tweets=5
        )
        for i, tweet in enumerate(keyword_tweets):
            logger.info(
                f"Keyword Tweet {i + 1}: ID={tweet.tweet_id}, User={tweet.user_handle}, Text='{tweet.text_content[:50]}...'"
            )
        ...
        profile_tweets = scraper.scrape_tweets_from_profile(
            profile_to_scrape, max_tweets=5
        )
        for i, tweet in enumerate(profile_tweets):
            logger.info(
                f"Profile Tweet {i + 1}: ID={tweet.tweet_id}, User={tweet.user_handle}, Text='{tweet.text_content[:50]}...'"
            )

        # Test 3: Scrape by hashtag
        hashtag_to_scrape = "#OpenAI"
        logger.info(f"\n--- Scraping for hashtag: {hashtag_to_scrape} ---")
        hashtag_tweets = scraper.scrape_tweets_by_hashtag(
            hashtag_to_scrape, max_tweets=5
        )
        for i, tweet in enumerate(hashtag_tweets):
            logger.info(
                f"Hashtag Tweet {i + 1}: ID={tweet.tweet_id}, User={tweet.user_handle}, Text='{tweet.text_content[:50]}...'"
            )
