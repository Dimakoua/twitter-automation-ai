...
class ConfigLoader:
    def __init__(
        ...
        self.settings: Dict[str, Any] = self._load_json(
            self.settings_file, default_value={}
        )
        self.accounts: List[Dict[str, Any]] = self._load_json(
            self.accounts_file, default_value=[]
        )

        if not self.settings:
            logger.warning(
                f"Settings file '{self.settings_file}' was not found or is empty/invalid. Using empty settings."
            )
        if not self.accounts:
            logger.warning(
                f"Accounts file '{self.accounts_file}' was not found or is empty/invalid. Using empty accounts list."
            )
        ...
        try:
            for key in keys:
                if isinstance(current_level, dict):
                    ...
                    logger.warning(
                        f"Invalid path '{path_str}' at key '{key}'. Expected a dictionary, found {type(current_level)}."
                    )
                    return default
            return current_level
        except KeyError:
            logger.debug(
                f"Setting '{path_str}' not found. Returning default: {default}"
            )
            return default
        except Exception as e:
            logger.warning(
                f"Error accessing setting '{path_str}': {e}. Returning default: {default}"
            )
            return default
...
if __name__ == "__main__":
    ...
    if not DEFAULT_SETTINGS_FILE.exists():
        dummy_settings = {
            ...
            "logging": {
                ...
                "file_handler": {"enabled": True, "path": "logs/app.log"},
            },
        }
    ...
    logger.info(
        f"NonExistent API Key: {loader.get_api_key('non_existent_key')}"
    )  # Test default

    logger.info(
        f"Media Directory: {loader.get_twitter_automation_setting('media_directory')}"
    )
    logger.info(
        f"Like Probability: {loader.get_twitter_automation_setting('engagement_options.like_probability')}"
    )  # Test nested with old method
    ...
    logger.info(
        f"Like Probability (new method): {loader.get_setting('twitter_automation.engagement_options.like_probability')}"
    )
    logger.info(
        f"Non-existent deep path: {loader.get_setting('a.b.c.d', 'default_value')}"
    )
    logger.info(
        f"Path to non-dict: {loader.get_setting('logging.level.sublevel', 'default_value_for_non_dict')}"
    )
    logger.info(
        f"Path to list item (not supported by this simple getter): {loader.get_setting('accounts.0.username')}"
