...
class FileMessageQueue:
    ...
    def _get_next_index(self):
        ...
        existing_processing = [f for f in os.listdir(self.processing_dir) if f.endswith(".json")]
                ...
                if files:
                    ...
                    try:
                        ...
                        if os.path.exists(destination_filename):
                            shutil.move(destination_filename, source_filename) # Try to move it back
        ...
        with self.lock:
            ...
            if os.path.exists(processing_filename):
                if requeue:
                    ...
                    current_retries = self._get_retries_from_message(processing_filename)
                    if current_retries < dead_letter_after_attempts - 1: # -1 because we increment below
                        # Increment retry count and move back to the main queue
                        message_content = None
                        with open(processing_filename, 'r') as f:
                            message_content = json.load(f)
                        message_content['__retries'] = message_content.get('__retries', 0) + 1
                        with open(processing_filename, 'w') as f:
                            json.dump(message_content, f)
                        shutil.move(processing_filename, os.path.join(self.queue_dir, message_id))
                        return True
                    else:
                        # Move to dead-letter queue
                        shutil.move(processing_filename, os.path.join(self.dead_letter_dir, message_id))
