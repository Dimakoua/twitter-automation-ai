...
def setup_logger(
    config_loader: Optional[ConfigLoader] = None, logger_name: Optional[str] = None
):
    """
    ...
    if console_handler_config.get("enabled", True):  # Enabled by default
        console_log_level_str = console_handler_config.get(
            "level", default_log_level_str
        ).upper()
    ...
    if file_handler_config.get(
        "enabled", False
    ):  # Disabled by default, explicit enable needed
        ...
        try:
            ...
            print(
                f"Error: Could not create log directory {log_file_path.parent}. File logging disabled. Error: {e}",
                file=sys.stderr,
            )
            return  # Exit if we can't create log dir

        file_log_level_str = file_handler_config.get(
            "level", default_log_level_str
        ).upper()
        ...
        # Rotation settings
        rotation_type = file_handler_config.get(
            "rotation_type", None
        )  # e.g., 'size', 'time'
        max_bytes = int(
            file_handler_config.get("max_bytes", 1024 * 1024 * 5)
        )  # 5MB default
        backup_count = int(file_handler_config.get("backup_count", 5))
        when = file_handler_config.get(
            "when", "midnight"
        )  # For TimedRotatingFileHandler
        interval = int(
            file_handler_config.get("interval", 1)
        )  # For TimedRotatingFileHandler
...
if __name__ == "__main__":
    ...
    another_logger = logging.getLogger(
        "MyTestModule"
    )  # Example of another named logger

    main_logger.debug(
        "This is a DEBUG message from __main__ logger. (Should appear in file, not console)"
    )
    main_logger.info(
        "This is an INFO message from __main__ logger. (Should appear in file and console)"
    )
    ...
    if (
        test_settings["logging"]["file_handler"]["enabled"]
        and test_settings["logging"]["file_handler"]["rotation_type"] == "size"
    ):
        for i in range(
            200
        ):  # Each message is ~100-150 bytes with format. 200 * 150B = 30KB > 10KB
            main_logger.debug(
                f"Rotation test message {i + 1} - This is a somewhat long line to fill up the log file quickly for testing purposes."
            )
        print(
            "Rotation test complete. Check 'logs' directory for 'test_app.log', 'test_app.log.1', etc."
        )
    else:
